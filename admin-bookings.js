...
<hr>        <div class="detail-row"><b>Email:</b> ${data.user_email}</div>        
...
<b>Activities:</b> ${(parsed.activities || [])
.map(a => `${a.name} ₹${a.price}`)
.join(", ") || "-"}
<br>
<b>Activities Total:</b> ₹${activitiesTotal}   
</div>        <div class="detail-row">        
...
</div>        <hr>        <h3>Cab Details</h3>        <div class="detail-row">        
Cab Number:        
...
</div>        <button onclick="saveCabDetails()">Save Cab</button>

<h3>Assign Hotel (4N / 5D)</h3>        <table style="width:100%;border-collapse:collapse">        <tr style="background:#f5f5f5">        
...
<button onclick="adminPreviewInvoice('${data.id}')">
Preview Invoice
</button>

<button onclick="adminDownloadInvoice('${data.id}')">
Download Invoice
</button>
` : `<div style="margin-top:10px;font-weight:600">
Invoice available after booking assigned
</div>`}
...
function adminPreviewInvoice(id){
localStorage.setItem("invoiceBookingId", id)
window.open("invoice.html", "_blank")
}

function adminDownloadInvoice(id){
localStorage.setItem("invoiceBookingId", id)
